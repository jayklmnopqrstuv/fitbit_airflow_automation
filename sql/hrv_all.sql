WITH
  ...
  SELECT
    participantid,
    AVG(beat_interval_ms_rmssd) AS mean_rmssd,
    AVG(beat_interval_ms_stddev) AS mean_sdrr
  FROM (
    SELECT
      ---PARSE_DATE("%F", '{{ ds }}') as ds,
      hr.participantid,
      /* Bucket timestamps on 5 minute intervals and compute
  heart rate variability statistics within each bucket. */ datetime(TIMESTAMP_SECONDS( UNIX_SECONDS(hr.hr_time) - MOD(UNIX_SECONDS(hr.hr_time), 5 * 60) )) AS datetime_local,
      stddev(60000.0 / hr.bpm) AS beat_interval_ms_stddev,
      SQRT(AVG(POW(60000.0*(1/hr.bpm-1/hr.prev_bpm), 2))) AS beat_interval_ms_rmssd
    FROM
      heart_rate AS hr
    ...
    GROUP BY
      ...
      datetime_local
    HAVING
      AVG(hr.bpm) IS NOT NULL)
  GROUP BY
    participantid),
  hrv_part_two AS (
  SELECT
    participantid,
    AVG(beat_interval_ms_mean) AS beat_interval_ms_mean,
    AVG(beat_interval_ms_stddev) AS beat_interval_ms_stddev,
    AVG(beat_interval_ms_min) AS beat_interval_ms_min,
    AVG(beat_interval_ms_max) AS beat_interval_ms_max,
    AVG(beat_interval_ms_rmssd) AS beat_interval_ms_rmssd,
    AVG(geom_poincare_sd1) AS geom_poincare_sd1,
    AVG(geom_poincare_sd2) AS eom_poincare_sd2,
    AVG(tinn) AS tinn,
    AVG(rrtri) AS rrtri,
    AVG(sdann) AS sdann
  FROM (
    ...
    ON
      ...
      AND A.start_time = C.start_time
    WHERE
      A.bpm_mean IS NOT NULL)
  GROUP BY
    participantid)
SELECT
  PARSE_DATE("%F", '{{ ds }}') as ds,
  a.participantid AS pim_id,
  a.*EXCEPT(participantid),
  b.*EXCEPT(participantid)
FROM
  hrv_part_one AS a
FULL OUTER JOIN
  hrv_part_two AS b
