(WITH
  ...
  WHERE
    DATE(heart_rate_datetime_loc) = DATE_SUB(PARSE_DATE("%F", '{{ ds }}'),INTERVAL 1 DAY)
    AND bpm >= 40 ),
  nn_interval_5min AS(
  SELECT
    participantid,
  ...
  SELECT
    participantid,
    datetime(TIMESTAMP_SECONDS( UNIX_SECONDS(start_time_5min) - MOD(UNIX_SECONDS(start_time_5min), 60 * 60) )) AS start_time,
    #1hour
  ...
  SELECT
    *,
    datetime(TIMESTAMP_SECONDS( UNIX_SECONDS(hr_time) - MOD(UNIX_SECONDS(hr_time), 60 * 60) )) AS start_time,
    #1hour
  ...
  SELECT
    ...
    /* Bucket timestamps on 1 day intervals and compute
               heart rate variability statistics within each bucket. */ datetime(TIMESTAMP_SECONDS( UNIX_SECONDS(hr.hr_time) - MOD(UNIX_SECONDS(hr.hr_time), 60 * 60) )) AS start_time,
    #1hour
    COUNT(*) AS record_count,
...
SELECT
  ...
  A.participantid AS pim_id,
  '1 hour' as hrv_type,
  A.start_time AS start_time_local,
  TIMESTAMP_ADD(A.start_time,INTERVAL 1 HOUR) AS end_time_local,
  A.record_count,
  A.bpm_mean,
  A.bpm_stddev,
  A.bpm_min,
...
ORDER BY
  ...
  SELECT
    participantid,
    datetime(TIMESTAMP_SECONDS( UNIX_SECONDS(start_time_5min) - MOD(UNIX_SECONDS(start_time_5min), 3 * 60 * 60) )) AS start_time,
    #1hour
    STDDEV(ave_nn) AS sdann,
    COUNT(*) AS record_count
  FROM
  ...
  SELECT
    *,
    datetime(TIMESTAMP_SECONDS( UNIX_SECONDS(hr_time) - MOD(UNIX_SECONDS(hr_time), 3 * 60 * 60) )) AS start_time,
    #1hour
    ROUND((beat_interval*10000 - MOD(CAST(beat_interval*10000 AS int64), 312500))/10000,3) AS histogram_bin
  FROM
    heart_rate),
  ...
  SELECT
    hr.participantid,
    /* Bucket timestamps on 1 day intervals and compute
               heart rate variability statistics within each bucket. */ datetime(TIMESTAMP_SECONDS( UNIX_SECONDS(hr.hr_time) - MOD(UNIX_SECONDS(hr.hr_time), 3 * 60 * 60) )) AS start_time,
    #1hour
    COUNT(*) AS record_count,
    AVG(hr.bpm) AS bpm_mean,
    stddev(hr.bpm) AS bpm_stddev,
...
PARSE_DATE("%F", '{{ ds }}') as ds,
  ...
  '3 hour' as hrv_type,
  A.start_time AS start_time_local,
  TIMESTAMP_ADD(A.start_time,INTERVAL 3 HOUR) AS end_time_local,
  A.record_count,
  A.bpm_mean,
  A.bpm_stddev,
  A.bpm_min,
...
ORDER BY
  ...
  SELECT
    participantid,
    datetime(TIMESTAMP_SECONDS( UNIX_SECONDS(start_time_5min) - MOD(UNIX_SECONDS(start_time_5min), 6 * 60 * 60) )) AS start_time,
    #1hour
    STDDEV(ave_nn) AS sdann,
    COUNT(*) AS record_count
  FROM
  ...
  SELECT
    *,
    datetime(TIMESTAMP_SECONDS( UNIX_SECONDS(hr_time) - MOD(UNIX_SECONDS(hr_time), 6 * 60 * 60) )) AS start_time,
    #1hour
    ROUND((beat_interval*10000 - MOD(CAST(beat_interval*10000 AS int64), 312500))/10000,3) AS histogram_bin
  FROM
    heart_rate),
  ...
  SELECT
    hr.participantid,
    /* Bucket timestamps on 1 day intervals and compute
               heart rate variability statistics within each bucket. */ datetime(TIMESTAMP_SECONDS( UNIX_SECONDS(hr.hr_time) - MOD(UNIX_SECONDS(hr.hr_time), 6 * 60 * 60) )) AS start_time,
    #1hour
    COUNT(*) AS record_count,
    AVG(hr.bpm) AS bpm_mean,
    stddev(hr.bpm) AS bpm_stddev,
...
PARSE_DATE("%F", '{{ ds }}') as ds,
  ...
  '6 hour' as hrv_type,           
  A.start_time AS start_time_local,
  TIMESTAMP_ADD(A.start_time,INTERVAL 6 HOUR) AS end_time_local,
  A.record_count,
  A.bpm_mean,
  A.bpm_stddev,
  A.bpm_min,
...
ORDER BY
  ...
  SELECT
    participantid,
    datetime(TIMESTAMP_SECONDS( UNIX_SECONDS(start_time_5min) - MOD(UNIX_SECONDS(start_time_5min), 12 * 60 * 60) )) AS start_time,
    #1hour
    STDDEV(ave_nn) AS sdann,
    COUNT(*) AS record_count
  FROM
  ...
  SELECT
    *,
    datetime(TIMESTAMP_SECONDS( UNIX_SECONDS(hr_time) - MOD(UNIX_SECONDS(hr_time), 12 * 60 * 60) )) AS start_time,
    #1hour
    ROUND((beat_interval*10000 - MOD(CAST(beat_interval*10000 AS int64), 312500))/10000,3) AS histogram_bin
  FROM
    heart_rate),
  ...
  SELECT
    hr.participantid,
    /* Bucket timestamps on 1 day intervals and compute
               heart rate variability statistics within each bucket. */ datetime(TIMESTAMP_SECONDS( UNIX_SECONDS(hr.hr_time) - MOD(UNIX_SECONDS(hr.hr_time), 12 * 60 * 60) )) AS start_time,
    #1hour
    COUNT(*) AS record_count,
    AVG(hr.bpm) AS bpm_mean,
    stddev(hr.bpm) AS bpm_stddev,
...
PARSE_DATE("%F", '{{ ds }}') as ds,
  ...
  '12 hour' as hrv_type,           
  A.start_time AS start_time_local,
  TIMESTAMP_ADD(A.start_time,INTERVAL 12 HOUR) AS end_time_local,
  A.record_count,
  A.bpm_mean,
  A.bpm_stddev,
  A.bpm_min,
